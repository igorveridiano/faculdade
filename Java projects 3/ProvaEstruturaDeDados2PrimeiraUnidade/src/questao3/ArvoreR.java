...
public class ArvoreR<Key extends Comparable<Key>, Value> {

	private Node<Key, Value> root;
	private int size;
	private FilaDupEnc<Node<Key, Value>> queue = new FilaDupEnc<>();
	private Collection<Value> collection1 = new ArrayList<>();;
	private Collection<String> collection2 = new ArrayList<>();;
		...
		if (size == 0) {
			...
			while (verification != true) {
				if (nodeKey.compareTo(currentNode.getKey()) == 0) {
					...
					for (String hora : newNode.listTime) {
						currentNode.listTime.add(hora);
					}
					verification = true;
	...
	public Collection<Value> obterTodos() {
		queue.limpar();
		collection1.clear();
		Node<Key, Value> currentNode = null;
		this.queue.enfileirar(getRoot());
		collection1.add(getRoot().getValue());
		while (!this.queue.vazia()) {
			currentNode = this.queue.desenfileirar();
			if (currentNode.getLeftSon() != null) {
				this.queue.enfileirar(currentNode.getLeftSon());
				collection1.add(currentNode.getLeftSon().getValue());
			}
			if (currentNode.getRightSon() != null) {
				this.queue.enfileirar(currentNode.getRightSon());
				collection1.add(currentNode.getRightSon().getValue());
			}
		}
		return collection1;
	}
	...
	public Collection<String> obterHoraRepeticao(Key chave) {
		collection2.clear();
		Node<Key, Value> currentNode = get(chave);
		collection2 = currentNode.listTime;
		return collection2;
	}
