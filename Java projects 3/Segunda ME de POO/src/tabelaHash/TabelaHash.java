...
public class TabelaHash<K, V> implements ITabelaHash<K, V> {
	...
	private class IteradorTabelaHash implements Iterator<V> {
		...
		private int posicao1, posicao2, posicao3, aux1;
		...
		public V next() {
			if (hasNext()) {
				if (aux1 > 0){
					ListaDuplamenteEncadeada<TabelaHashNo> lista = tabela.obter(posicao3);
					if(lista != null){
				...
				posicao3 = posicao1;
				ListaDuplamenteEncadeada<TabelaHashNo> lista = tabela.obter(posicao1++);
				posicao2 = 0;
				aux1 = 0;
				if(lista != null){
					while(posicao2 >= 0 && posicao2 < lista.tamanho()) {
