...
public class Grafo implements IGrafo {
	...
	public void algoritmoDijkstraDirecionado(Vertice inicio) {
		ArrayList<ElementoMenorCaminho> lista = new ArrayList<>();
		Vertice atual;
		boolean auxiliar;
		ElementoMenorCaminho elementoAux;
		...
		while (!this.queue.isEmpty()) {
			auxiliar = true;
			atual = this.queue.poll();
			getMenorCaminhoDirecionado(atual, lista);
			for (int i = 0; i < lista.size(); i++) {
				for (int j = 0; j < lista.size(); j++) {
					if (lista.get(i).getDistancia() < lista.get(j).getDistancia()) {
						elementoAux = lista.get(i);
	...
	private void getMenorCaminhoDirecionado(Vertice atual, ArrayList<ElementoMenorCaminho> lista) {
		double distancia = 0;
		boolean auxiliar;
		for (Vertice vertice : getAdjacentes(atual)) {
			auxiliar = false;
