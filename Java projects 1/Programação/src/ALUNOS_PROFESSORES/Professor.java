...
public class Professor extends Pessoa {
	...
	public String toString() {
		...
		saida = String.format("%-20s %-35s %-10s %-10s\n", this.getMatricula(), this.getNome(),
				this.getQuantidade_Creditos(), this.getValor_hora_aula());
		/*
		 * StringBuilder string =new StringBuilder();
		 * string.append(this.getMatricula()); while(string.length()<21){
		 * string.append(" "); } string.append(this.getNome());
		 * while(string.length()<65){ string.append(" "); }
		 * string.append(this.getQuantidade_Creditos());
		 * while(string.length()<75){ string.append(" "); }
		 * string.append(this.getQuantidade_Creditos()); String saida; saida=new
		 * String(string);
		 */
		return saida;
